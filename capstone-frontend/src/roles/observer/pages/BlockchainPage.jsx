...
import { Shield, CheckCircle, XCircle, Link2, Clock, Minus } from 'lucide-react';
...
export default function BlockchainPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-semibold text-gray-900">Blockchain Audit Trail</h1>
        <p className="text-sm text-gray-500 mt-1">Immutable event chain verification for procurement transparency.</p>
      </div>
      ...
      {chainStatus && (
        <div className={`rounded-xl border p-5 flex items-center gap-4 ${chainStatus.valid ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          ...
          <div>
            <p className="font-semibold text-sm">{chainStatus.valid ? 'Chain Integrity Verified' : 'Chain Integrity FAILED'}</p>
            <p className="text-xs text-gray-600 mt-0.5">{chainStatus.blocks_checked ?? 0} blocks checked &bull; {chainStatus.errors ?? 0} error(s) detected</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">Blockchain Events</h2>
        </div>
        {loading ? (
          <div className="p-8 text-center text-gray-400">Loading blockchain events...</div>
        ) : events.length === 0 ? (
          <div className="p-8 text-center text-gray-400">No blockchain events recorded yet.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-left">
                <tr>
                  ...
                  <th className="px-3 py-2.5 font-medium text-gray-600">Event Type</th>
                  ...
                  <th className="px-3 py-2.5 font-medium text-gray-600">Recorded</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {events.map((evt) => (
                  <tr key={evt.id} className={`hover:bg-gray-50 cursor-pointer ${selectedEvent?.id === evt.id ? 'bg-blue-50' : ''}`}
                    ...
                    <td className="px-3 py-2.5 font-mono text-[10px] text-gray-400 max-w-[120px] truncate" title={evt.block_hash}>
                      {evt.block_hash?.substring(0, 16)}...
                    </td>
                    <td className="px-3 py-2.5">
                      {evt.eth_tx_hash ? (
